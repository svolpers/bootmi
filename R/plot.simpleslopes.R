...
plot.simpleslopes <- function( x, ... ) {
  ...
  dev.new(); par(mar = c(5,4,4,9))
  # plot start and end points of slopes
  plot( xv, yv, main=title, xlab=x$info$X, ylab=x$info$Y, ylim=c((min(yv)),(max(yv))), xlim=c((min(xv)),(max(xv))), bty="L"
  )
  # extract position of starting points 
  i = c(1:length(x))[c(T,F)]
  # connect corresponding dots by lines
  segments(xv[i],yv[i],xv[i+1],yv[i+1],lty=1:length(i))
  # add legend
