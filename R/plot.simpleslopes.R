...
plot.simpleslopes <- function( x, ... ) {
  ...
  graphics::plot( xv, yv, main=title, xlab=x$info$X, ylab=x$info$Y, 
    ylim=c((min(yv)),(max(yv))), xlim=c((min(xv)),(max(xv))), bty="L"
  )
  ...
  # add legend
  graphics::legend( x="topright", legend = mod_vals, inset=c(-.40,0), lty = 1:length(i), cex = .8, ncol = 1, title = "Moderator Values", xpd=TRUE)
}
