...
simslop <- function( object, x_var, m_var, m2_var= NULL, ci=95, mod_values_type=c("sd","val"), mod_values=c(-1,0,1), centered=FALSE, dat_org= NULL) {
    UseMethod("simslop")
}

#' @rdname simslop
#' @export
simslop.default <- function( object, x_var, m_var, m2_var= NULL, ci=95, mod_values_type=c("sd","val"), mod_values=c(-1,0,1), centered=FALSE, dat_org= NULL) {
  ...
  if(mod_values_type == "sd") {
    value_x_h = mean( object$dat[,x_var], na.rm = TRUE) + sd( object$dat[,x_var], na.rm = TRUE)
    value_x_l = mean( object$dat[,x_var], na.rm = TRUE) - sd( object$dat[,x_var], na.rm = TRUE)
  } else {
  ...
  # create empty matrix
  if( !is.null(m2_var) ) {
    simslop = data.frame( matrix( rep(0,6*amnt_mod_vals^(1+length(m2_var))), ncol = 6))
    colnames(simslop) = c("value_m","value_m2","slope","se","y_l","y_h")
  } else {
    simslop = data.frame( matrix( rep(0,5*amnt_mod_vals^(1+length(m2_var))), ncol = 5))
  ...
  for(i in seq( amnt_mod_vals)) {
    ...
    if( mod_values_type == "sd" ) { # && mod_values[i] != 0
      value_m = mean( object$dat[ ,m_var], na.rm = TRUE) + (mod_values[i]) * sd( object$dat[ ,m_var], na.rm = TRUE)
    } else {
      value_m = mod_values[i]
    }

    # value_m = mean( object$dat[ ,m_var]) + (mod_values[1]) * sd( object$dat[ ,m_var])
    ...
    if( !is.null(m2_var) ) {
      ...
      for(j in seq( amnt_mod_vals)) {
        ...
        if( mod_values_type == "sd" ) {     
          value_m2 = mean( object$dat[ ,m2_var], na.rm = TRUE) + (mod_values[j]) * sd( object$dat[ ,m2_var], na.rm = TRUE)
        } else {
...
#' @rdname simslop
#' @export
simslop.bootmi.lm <- function( object, x_var, m_var, m2_var= NULL, ci=95, mod_values_type = "sd", mod_values = c(-1,0,1), centered=TRUE, dat_org= NULL) {
